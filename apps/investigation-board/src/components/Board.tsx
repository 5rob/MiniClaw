...
interface BoardProps {
  ...
  onConnectionClick: (connections: Connection[], topicName: string) => void;
}
...
const Board: React.FC<BoardProps> = ({
  ...
  const initialNodes = useMemo(() => {
    ...
    // Helper: check if entity has any data points in active topics
    const hasVisibleData = (entityId: string): boolean => {
      return dataPoints.some(
        (dp) => dp.entityId === entityId && filters.activeTopics.has(dp.topicName)
      );
    };

    // Filter entities
    const visibleEntities = entities.filter((entity) => {
      // Hide minor entities if toggled off
      if (entity.category === 'minor' && !filters.showMinorEntities) {
        return false;
      }
      
      // Hide entities with no visible data in active topics
      if (!hasVisibleData(entity.id)) {
        return false;
      }
    ...
    const centerX = 500;
    const centerY = 400;
    const primaryRadius = 500; // Increased from 350
    const minorRadius = 850; // Increased from 600
  ...
  const initialEdges = useMemo(() => {
    ...
    const visibleEntityIds = new Set(
      entities
        .filter((entity) => {
          ...
          // Check if entity has visible data
          const hasData = dataPoints.some(
            (dp) => dp.entityId === entity.id && filters.activeTopics.has(dp.topicName)
          );
          return hasData;
        })
        .map((e) => e.id)
    ...
    connections.forEach((conn) => {
      // Skip if either entity is not visible
      if (!visibleEntityIds.has(conn.sourceEntityId) || !visibleEntityIds.has(conn.targetEntityId)) {
        return;
      }
    ...
    return edges;
  }, [entities, connections, topics, filters, dataPoints, onConnectionClick]);
