"""
Benchmark WITHOUT flash-attn on GPU to see if flash-attn actually helps.
"""
import os
import sys
...
p("flash-attn: BLOCKED")

p(f"\nLoading 1.7B model on GPU (SDPA attention)...")
t0 = time.time()
...
p(f"\n{'='*60}")
p(f"GPU RESULTS (1.7B + SDPA + bfloat16) — NO FLASH-ATTN")
p(f"{'='*60}")
