"""
Benchmark TTS with flash-attn installed.
Quick test with 1 phrase per mode.
"""
import os
import sys
...
sf.write(os.path.join(OUTPUT_DIR, "bench_xvec_flash.wav"), audio, sr)

# ICL mode
p(f"\n--- ICL mode ---")
torch.cuda.empty_cache()
t0 = time.time()
vp2 = model.create_voice_clone_prompt(VOICE_REF, ref_text="Hey there, how's your evening going?")
p(f"Voice cached: {time.time()-t0:.2f}s")

torch.cuda.empty_cache()
p(f"Generating...")
t0 = time.time()
audios2, sr2 = model.generate_voice_clone(text=phrase, voice_clone_prompt=vp2)
gen_time2 = time.time() - t0
audio2 = audios2[0]
dur2 = len(audio2) / sr2
p(f"Time: {gen_time2:.2f}s | Audio: {dur2:.2f}s | RTF: {gen_time2/dur2:.2f}x")
sf.write(os.path.join(OUTPUT_DIR, "bench_icl_flash.wav"), audio2, sr2)

p(f"\nDone. x_vec={gen_time:.2f}s, ICL={gen_time2:.2f}s")
