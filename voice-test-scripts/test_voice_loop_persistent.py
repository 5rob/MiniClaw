...
Models are loaded once and kept in memory for fast subsequent calls.
"""
...
def get_tts_model():
    ...
    if _tts_model is None:
        ...
        start = time.time()
        _tts_model = Qwen3TTSModel.from_pretrained(TTS_MODEL_PATH)
...
def get_asr_model():
    ...
    if _asr_model is None:
        ...
        start = time.time()
        _asr_model = Qwen3ASRModel.from_pretrained(ASR_MODEL_PATH)
