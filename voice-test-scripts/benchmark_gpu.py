"""
Benchmark with model explicitly on GPU.
The model was loading on CPU - that's why RTF was 6x!
"""
import os
import sys
...
# Load with device_map to force GPU
...
for i, phrase in enumerate(phrases):
    ...
    outfile = os.path.join(OUTPUT_DIR, f"bench_gpu_{i}.wav")
    sf.write(outfile, audio, sr)
    p(f"  Time: {gen_time:.2f}s | Audio: {dur:.2f}s | RTF: {rtf:.2f}x")

avg = sum(times) / len(times)
p(f"\n{'='*60}")
p(f"GPU RESULTS (1.7B + flash-attn + bfloat16)")
p(f"{'='*60}")
