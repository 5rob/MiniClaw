...
// v2.1: Fixed image generation model (gemini-2.0-flash-exp → gemini-2.0-flash-001)
import fs from 'fs';
...
export async function describeImage(imageUrl, mimeType) {
  ...
  try {
    ...
    const response = await fetch(`${VISION_URL}?key=${GEMINI_API_KEY}`, {
      method: 'POST',
...
// --- Image Generation ---
const GENERATION_MODEL = 'gemini-2.0-flash-001'; // Supports native image generation with responseModalities
...
export async function generateImage(prompt, aspectRatio = '1:1') {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { error: `Gemini API error: ${response.status} — ${errorText.slice(0, 200)}` };
    }
